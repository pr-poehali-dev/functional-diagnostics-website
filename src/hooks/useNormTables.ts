...
function convertFromApi(apiTable: ApiNormTable): NormTable {
  return {
    ...
    rows: apiTable.rows.map((row: any) => ({
      id: row.id,
      rangeFrom: row.rangeFrom || row.range_from,
      rangeTo: row.rangeTo || row.range_to,
      rangeUnit: row.rangeUnit || row.range_unit,
      parameterFrom: row.parameterFrom || row.parameter_from,
      parameterTo: row.parameterTo || row.parameter_to,
      borderlineLow: row.borderlineLow || row.borderline_low,
      borderlineHigh: row.borderlineHigh || row.borderline_high,
    })),
